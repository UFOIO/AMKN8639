...
/* Hard-coded USART1 addresses - no pointer arithmetic */
...
void Reset_Handler(void){
    ...
    /* TEST1: read ISR once, no loop */
    isr_val = USART1_ISR;
    /* TEST2: wait TXE with max attempts */
    for(i=0;i<1000000;i++){
        if(USART1_ISR & (1<<7)) break;
    }
    /* TEST3: write if TXE was set */
    USART1_TDR = 'X';
