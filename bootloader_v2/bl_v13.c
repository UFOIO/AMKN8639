/* AMKN8639 V13 - GPIO/CS Monitor + Clear AHB3LPENR + Pin toggle test */
...
#define QSPI_PSMKR      (*(volatile uint32_t*)0x52005024)
#define QSPI_PSMAR      (*(volatile uint32_t*)0x52005028)
#define QSPI_PIR         (*(volatile uint32_t*)0x5200502C)
#define QSPI_LPTR       (*(volatile uint32_t*)0x52005030)
...
#define PR(fmt, v) do{uart_puts(fmt"=");uart_puthex(v);uart_puts("\r\n");}while(0)
...
void Reset_Handler(void) {
    ...
    led_blip(30); led_blip(30); delay(640000);
    ...
    /* Clear boot ROM anomaly */
    RCC_AHB3LPENR = 0;
    PR("AHB3LPENR(clr)", RCC_AHB3LPENR);
    ...
    /* GPIO config: PF8(AF9), PF9(AF9), PF10(AF9), PG6(AF9) */
    ...
    PR("GPIOF_PUPDR ", GPIOF_PUPDR);
    PR("GPIOF_OSPEEDR", GPIOF_OSPEEDR);
    PR("GPIOF_AFRL  ", GPIOF_AFRL);
    PR("GPIOF_AFRH  ", GPIOF_AFRH);
    PR("GPIOG_MODER ", GPIOG_MODER);
    PR("GPIOG_PUPDR ", GPIOG_PUPDR);
    PR("GPIOG_AFRH  ", GPIOG_AFRH);
    
    /* Check pin states before QSPI */
    PR("GPIOF_IDR   ", GPIOF_IDR);
    PR("GPIOG_IDR   ", GPIOG_IDR);
    uint32_t pg6 = GPIOG_IDR & (1<<6);
    uart_puts("PG6(CS)="); uart_puts(pg6 ? "HI\r\n" : "LO\r\n");
    uint32_t pf10 = GPIOF_IDR & (1<<10);
    uart_puts("PF10(CLK)="); uart_puts(pf10 ? "HI\r\n" : "LO\r\n");
    ...
    PR("QSPI_CR(pre) ", QSPI_CR);
    PR("QSPI_SR(pre) ", QSPI_SR);
    
    /* Check CS state before enable */
    pg6 = GPIOG_IDR & (1<<6);
    uart_puts("PG6(preEN)="); uart_puts(pg6 ? "HI " : "LO ");
    
    /* Enable QSPI */
    QSPI_CR |= 1;
    __asm("dsb");
    
    /* Check CS state immediately after enable */
    pg6 = GPIOG_IDR & (1<<6);
    uart_puts("PG6(postEN)="); uart_puts(pg6 ? "HI\r\n" : "LO\r\n");
    
    delay(1000);
    PR("QSPI_SR(1ms) ", QSPI_SR);
    PR("QSPI_CR(1ms) ", QSPI_CR);
    
    pg6 = GPIOG_IDR & (1<<6);
    uart_puts("PG6(1ms)="); uart_puts(pg6 ? "HI\r\n" : "LO\r\n");
    
    /* Wait longer */
    volatile int to = 20000000;
    while(--to) {
        if(QSPI_SR & (1<<1)) break;
    }
    PR("QSPI_SR(end) ", QSPI_SR);
