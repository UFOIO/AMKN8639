...
void Reset_Handler(void) {
    ...
    /* Flash reset: exit QPI mode, back to SPI */
    uart_puts("--- Flash Reset ---\r\n");
    /* CR already set. Set CCR for indirect write, send 0x66+0x99 */
    QSPI_CR &= ~1; delay(10);
    /* Send 0x66 (Enable Reset) */
    QSPI_CCR = 0x66 | (1<<8) | (0<<26); QSPI_DLR=0; QSPI_AR=0;
    QSPI_CR |= 1; delay(64000);
    QSPI_CR &= ~1; delay(10);
    QSPI_FCR = 0x1F;
    /* Send 0x99 (Reset Device) */
    QSPI_CCR = 0x99 | (1<<8) | (0<<26); QSPI_DLR=0; QSPI_AR=0;
    QSPI_CR |= 1; delay(640000);
    QSPI_CR &= ~1; delay(10);
    QSPI_FCR = 0x1F;
    /* Also send 0xAB (Release from Deep Power Down) */
    QSPI_CCR = 0xAB | (1<<8) | (0<<26); QSPI_DLR=0; QSPI_AR=0;
    QSPI_CR |= 1; delay(64000);
    QSPI_CR &= ~1; delay(10);
    QSPI_FCR = 0x1F;
