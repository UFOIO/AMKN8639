...
/* QSPI v6c: EN always on, trigger by AR, direct register writes */
static uint32_t qspi_jedec_id(void) {
    uint32_t id = 0;
    volatile uint32_t sr, cr, tmp;

    /* ABORT any previous xfer */
    cr = QSPI_CR;
    tmp = cr | (1<<1);  /* Set ABORT */
    *(volatile uint32_t*)0x52005000 = tmp;
    __asm("dsb");
    cr = QSPI_CR;
    uart_print("  CR_pre=", cr);

    /* Wait for BUSY=0 */
    for(int w=0;w<10000;w++) {
        sr = QSPI_SR;
        if(!(sr & (1<<5))) break;  /* BUSY clear */
    }
    ...
    cr = QSPI_CR;
    cr &= ~1;
    *(volatile uint32_t*)0x52005000 = cr;
    __asm("dsb");
    delay(100);

    /* Set CCR: JEDEC ID, IMODE=1, DMODE=1, FMODE=1 (indirect read) */
    uint32_t ccr = 0x9F | (1<<8) | (1<<24) | (1<<26);
    *(volatile uint32_t*)0x52005014 = ccr;
    ...
    *(volatile uint32_t*)0x52005010 = 2;
    uart_print("  DLR=", *(volatile uint32_t*)0x52005010);

    /* Enable (EN=1 + keep other bits) */
    cr = QSPI_CR;
    cr |= 1;
    *(volatile uint32_t*)0x52005000 = cr;
    __asm("dsb");
    uart_print("  CR_en=", QSPI_CR);
