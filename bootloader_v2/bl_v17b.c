...
void Reset_Handler(void) {
    ...
    uart_puts("\r\n=== AMKN8639 MCUBoot V11 ===\r\n");
    crc32_init();
    /* PLL: HSI 64MHz -> VCO 480MHz -> SYSCLK 240MHz */
    (*(volatile uint32_t*)0x58024818) |= (1<<14);    /* PWR_D3CR VOS=1 */
    while(!((*(volatile uint32_t*)0x58024804) & (1<<14))) {} /* ACTVOSRDY */
    (*(volatile uint32_t*)0x58024428) = (3<<4);      /* PLLCKSELR DIVM=4 */
    (*(volatile uint32_t*)0x5802442C) = (1<<1)|(3<<2)|(29<<4); /* PLLCFGR */
    (*(volatile uint32_t*)0x58024430) = (1<<9);      /* PLL1DIVR DIVP=2 */
    RCC_CR |= (1<<24);                                /* PLL1ON */
    while(!(RCC_CR & (1<<25))) {}                     /* PLL1RDY */
    (*(volatile uint32_t*)(RCC_BASE+0x18)) = 0;      /* D1CFGR */
    (*(volatile uint32_t*)(RCC_BASE+0x1C)) = 0;      /* D2CFGR */
    { uint32_t t = (*(volatile uint32_t*)(RCC_BASE+0x10)); t &= ~(7<<0); t |= (3<<0); (*(volatile uint32_t*)(RCC_BASE+0x10)) = t; }
    while(((*(volatile uint32_t*)(RCC_BASE+0x10)) & (7<<3)) != (3<<3)) {}
    USART1_BRR = 240000000 / 115200;
    uart_puts("PLL OK @240MHz\r\n");
