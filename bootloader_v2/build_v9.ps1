# AMKN8639 BL V9 Build - Link ZQLY library for QSPI
param()
...
$Src    = Join-Path $BLDir "bl_v9.c"
$Obj    = Join-Path $BLDir "bl_v9.o"
$Elf    = Join-Path $BLDir "bl_v9.axf"
$Bin    = Join-Path $BLDir "bl_v9.bin"
$PadBin = Join-Path $BLDir "bl_v9_pad.bin"
$Sct    = Join-Path $BLDir "bootloader_v2.sct"

Write-Host "=== AMKN8639 BL V9 Build (ZQLY lib) ===" -ForegroundColor Cyan

# Step 1: Compile
Write-Host "[1/3] Compiling bl_v9.c..." -ForegroundColor Yellow
$ArmCCArgs = @(
...
if ($ccExit -ne 0 -and -not (Test-Path $Obj)) {
    Write-Error "armcc failed (no .o)"; exit 1
}
# armcc may exit 1 on warnings; accept if .o exists
if (Test-Path $Obj) {
    Write-Host "  [OK] armcc ($((Get-Item $Obj).Length) bytes)" -ForegroundColor Green
} else {
    Write-Error "armcc failed: no output file"; exit 1
}
...
if ($linkOut) { $linkOut | ForEach-Object { Write-Host $_ -ForegroundColor DarkGray } }
if ($linkExit -ne 0) { Write-Error "armlink failed (code=$linkExit)"; exit 1 }
...
Write-Host "  [OK] bl_v9.bin: $blSize bytes" -ForegroundColor Green
...
Write-Host "  [OK] bl_v9_pad.bin: $padSize bytes" -ForegroundColor Green
