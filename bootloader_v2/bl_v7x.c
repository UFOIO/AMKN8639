...
static uint32_t qspi_jedec_id(void) {
    ...
    /* Keep EN=1. ABORT stops any ongoing xfer */
    QSPI_CR |= (1<<1);   /* ABORT=1 */
    __asm("dsb");
    delay(100);
    QSPI_FCR = 0x1F;     /* Clear all flags incl TCF from ABORT */
    __asm("dsb");
    QSPI_CR &= ~(1<<1);  /* Clear ABORT bit, EN stays 1 */
    delay(100);
    /* Double-check TCF cleared */
    if(QSPI_SR & (1<<1)) {
        QSPI_FCR = (1<<1);
        __asm("dsb");
    }
    /* FCR already cleared flags above, no need to drain */

    /* Clear flags */
    QSPI_FCR = 0x1F;
    __asm("dsb");

    /* EN should be 1 from init */
    uart_print("  CR_in=", QSPI_CR);
    uart_print("  SR_in=", QSPI_SR);
    ...
    QSPI_CCR = 0x9F             /* JEDEC ID command */
             ...
             | (1 << 18)        /* DCYC = 1 - STM32H7 silicon fix! */
    ...
    /* EN already 1. AR write triggers xfer when FMODE=indirect read */
    QSPI_AR = 0;
    __asm("dsb");
    QSPI_DLR = 2;
    __asm("dsb");
    uart_print("  DLR_set=", QSPI_DLR);
    uart_print("  SR_go=", QSPI_SR);
