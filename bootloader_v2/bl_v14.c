/* AMKN8639 V14 - GPIO Force-High before QSPI AF + Full RCC Reset */
...
#define GPIOF_OTYPER    (*(volatile uint32_t*)0x58021404)
...
#define GPIOF_ODR       (*(volatile uint32_t*)0x58021414)
...
#define GPIOG_OTYPER    (*(volatile uint32_t*)0x58021804)
...
#define GPIOG_ODR       (*(volatile uint32_t*)0x58021814)
...
#define PR(f, v) do{uart_puts(f"=");uart_puthex(v);uart_puts("\r\n");}while(0)
...
void Reset_Handler(void) {
    ...
    uart_puts("\r\n=== V14 Force-High + Full Reset ===\r\n");
    
    /* Enable GPIO clocks */
    RCC_AHB4ENR |= (1<<5)|(1<<6);
    
    /* ===== Step 1: Force QSPI pins HIGH as GPIO ===== */
    uart_puts("Step 1: GPIO force-high\r\n");
    
    /* PF8, PF9, PF10, PG6 ? GPIO output */
    GPIOF_MODER &= ~((3<<16)|(3<<18)|(3<<20));
    GPIOF_MODER |= ((1<<16)|(1<<18)|(1<<20));  /* GPIO output */
    GPIOG_MODER &= ~(3<<12);
    GPIOG_MODER |= (1<<12);                     /* GPIO output */
    
    /* Set all HIGH */
    GPIOF_BSRR = (1<<8) | (1<<9) | (1<<10);    /* PF8, PF9, PF10 HIGH */
    GPIOG_BSRR = (1<<6);                        /* PG6 HIGH */
    __asm("dsb"); delay(100);
    
    PR("GPIOF_IDR(hi)", GPIOF_IDR);
    PR("GPIOG_IDR(hi)", GPIOG_IDR);
    
    /* ===== Step 2: Full RCC-level QSPI reset with clock OFF then ON ===== */
    uart_puts("Step 2: RCC QSPI hard reset\r\n");
    
    /* Assert QSPI reset */
    RCC_AHB3RSTR |= (1<<14);
    __asm("dsb"); delay(100);
    
    /* Disable QSPI clock entirely */
    RCC_AHB3ENR &= ~(1<<14);
    ...
    /* ===== Step 3: Now switch to AF mode ===== */
    uart_puts("Step 3: Switch to AF9\r\n");
    ...
    PR("GPIOF_IDR(af)", GPIOF_IDR);
    PR("GPIOG_IDR(af)", GPIOG_IDR);
    uint32_t pg6 = GPIOG_IDR & (1<<6);
    uart_puts("PG6(CS)="); uart_puts(pg6 ? "HI\r\n" : "LO\r\n");
    
    /* ===== Step 4: QSPI Init ===== */
    uart_puts("Step 4: QSPI Init\r\n");
    ...
    pg6 = GPIOG_IDR & (1<<6);
    uart_puts("PG6(pre)="); uart_puts(pg6 ? "HI " : "LO ");
    
    QSPI_CR |= 1;
    __asm("dsb");
    
    pg6 = GPIOG_IDR & (1<<6);
    uart_puts("PG6(post)="); uart_puts(pg6 ? "HI " : "LO ");
    
    volatile int to = 5000000;
    while(--to) { if(QSPI_SR & (1<<1)) break; if(QSPI_SR & (1<<0)) break; }
